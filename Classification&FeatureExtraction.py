...
train = []
j = []
df = pd.read_csv("TRAINEDCONTENT.csv", header = None,error_bad_lines=False,encoding='utf-8')
...
while i < 480:
    try:
   	...
   	print(df[2][i])     
       # df[0][i] =df[0][i].replace(' ','')

        req = requests.get(df[0][i],  headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.95 Safari/537.36'})
	...
	if isinstance(df[1][i], float):
	    ...
	    j +=1
    	    continue
        train.append((df[1][i],df[2][i]))
    #	df[2][i] = get_title(req.text)
#	print(df[1][i])
        i += 1
...
while i <len(af):
    try:
        #af.URL[i]=af.URL[i].replace(' ','')
	print(af.URL[i])
        req = requests.get(af.URL[i],  headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.95 Safari/537.36'})
        html = html2text.html2text(req.text)
...
#df.to_csv("download_dir.csv",encoding='utf-8',mode = 'w', index=False)
'''
i= 0
...
tf.to_csv('attr2.csv',encoding='utf-8',mode = 'w', index=False)
print(j)
